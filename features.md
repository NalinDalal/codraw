...
## Week 22: Aug 15, 2026 — Manager Extraction & Interaction Fixes

### Refactor

Deeper extraction of the drawing engine: all pointer/touch/keyboard/web-socket handlers and navigation/tool state moved out of `Game.ts` into dedicated managers, with two latent bugs fixed along the way.

| Time | Change                                             | Details                                                                                                                                                                                                                                             |
| ---- | -------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| —    | Pointer interaction isolation                      | Single-click/drag/resize/rotate dispatch plus shape-tool gestures moved into `PointerInteractionManager`; gesture state consolidated (`isDragging`/`isSelecting`/`isResizing`/`isRotating`) on one object instead of split between Game and context |
| —    | Mouse / touch / keyboard isolation                 | `mousedown`/`mouseup`/`mousemove`/`dblclick`/wheel, pinch-zoom & double-tap, and all shortcuts moved into `MouseManager`, `TouchManager`, `KeyboardManager`                                                                                         |
| —    | WebSocket diff sync isolation                      | Diff computation, legacy full-state handling, and broadcast moved into `WebSocketSyncManager`                                                                                                                                                       |
| —    | Navigation & tool state isolation                  | `NavigationManager` (zoom/pan/fit, minimap, frame slides, shape search) and `ToolManager` (tool switching, hand mode, lock, tool-change callback) extracted from Game                                                                               |
| —    | Clipboard, image-picker, edit-action consolidation | Paste listener moved into `ClipboardManager`; file-picker logic deduped in `ImageManager`; Enter-key edit action and arrow-label editing unified with the double-click editor in `shapeEdit.ts` (removed a duplicated arrow-label block)            |
| —    | Style-change callback fix                          | `setStyleChangeCallback` wrote a Game field while the eyedropper read it from the context bag — the callback never fired. Wired the pointer api to the same source (`styleChangeCallback` now reaches the UI)                                       |
| —    | Lock shortcut parity                               | `KeyboardManager` had a private `toggleLock()` that skipped the pointer-gesture reset. Ctrl+L now routes through `Game.toggleLock`, clearing `isDragging`/`isResizing`/etc. like the toolbar toggle                                                 |
| —    | Paste + Escape handling                            | Paste handler moved off Game into `ClipboardManager` (listener removed on destroy); `escapePressed` is set on every Escape and reset at the start of each pointer gesture, so Escape mid-drag aborts instead of committing a half-drawn shape       |
| —    | Result                                             | `Game.ts` reduced from 2056 → 1404 lines; 23 files under `draw/managers/`, `Game.ts` left as a thin public-API façade delegating to managers (only `copySelectionAsPng` remains inline by design)                                                   |
