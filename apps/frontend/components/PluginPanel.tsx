...
export function PluginPanel({
    ...
    return (
        <div className="fixed right-4 top-14 w-80 bg-black/80 backdrop-blur-md rounded-xl border border-white/10 p-4 text-white select-none z-20 max-h-[80vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-3">
                <div className="text-xs text-white/50 uppercase tracking-wider">Plugins</div>
                <button onClick={onClose} className="text-white/40 hover:text-white text-sm cursor-pointer">✕</button>
            </div>

            {/* Load plugin from code */}
            <div className="mb-3">
                <div className="text-xs text-white/60 mb-1">Load Plugin from Code</div>
                <input
                    type="text"
                    value={pluginName}
                    onChange={e => setPluginName(e.target.value)}
                    placeholder="Plugin name..."
                    className="w-full bg-white/10 border border-white/20 rounded px-2 py-1 text-xs text-white placeholder:text-white/30 mb-1"
                />
                <textarea
                    value={pluginCode}
                    onChange={e => setPluginCode(e.target.value)}
                    placeholder={`{\n  "id": "my-plugin",\n  "name": "My Plugin",\n  "version": "1.0.0",\n  "tools": [...]\n}`}
                    className="w-full h-24 bg-white/10 border border-white/20 rounded px-2 py-1 text-xs text-white placeholder:text-white/30 font-mono"
                />
                <button
                    onClick={handleLoadPlugin}
                    className="w-full mt-1 px-2 py-1 bg-blue-500 hover:bg-blue-600 rounded text-xs cursor-pointer"
                >
                    Load Plugin
                </button>
            </div>
            ...
            <div className="space-y-2">
                {plugins.length === 0 && (
                    <div className="text-xs text-white/30 text-center py-2">No plugins loaded</div>
                )}
                {plugins.map(plugin => (
                    <div key={plugin.id} className="bg-white/5 rounded p-2">
                        <div className="flex items-center justify-between mb-1">
                            <div className="text-xs font-medium">{plugin.name}</div>
                            <div className="text-[10px] text-white/40">v{plugin.version}</div>
                        </div>
                        <div className="text-[10px] text-white/40 mb-2">{plugin.id}</div>

                        {/* Plugin tools */}
                        {plugin.tools && plugin.tools.length > 0 && (
                            <div className="mb-2">
                                <div className="text-[10px] text-white/50 mb-1">Tools:</div>
                                <div className="flex flex-wrap gap-1">
                                    {plugin.tools.map(tool => (
                                        <button
                                            key={tool.id}
                                            onClick={() => handleActivateTool(tool.id)}
                                            className="px-2 py-0.5 bg-blue-500/20 hover:bg-blue-500/40 border border-blue-500/30 rounded text-[10px] cursor-pointer"
                                            title={tool.name}
                                        >
                                            {tool.name}
                                        </button>
                        ...
                        <div className="flex gap-1">
                            <button
                                ...
                                className="flex-1 px-1 py-0.5 bg-white/10 hover:bg-white/20 rounded text-[10px] cursor-pointer"
                            >
                                Export
                            </button>
                            <button
            ...
            <div className="mt-3 pt-3 border-t border-white/10">
                <div className="text-xs text-white/50 mb-2">Demo Plugins</div>
                <div className="space-y-1">
                    <button
                        ...
                        className="w-full px-2 py-1 bg-white/5 hover:bg-white/10 border border-white/10 rounded text-xs text-left cursor-pointer"
                    >
                        🎨 Spray Paint
                    </button>
                    <button
