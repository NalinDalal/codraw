...
export function Tooltip({
    ...
    return (
        <span className={`relative inline-flex group/tip ${className ?? ""}`}>
            ...
            <span
                role="tooltip"
                className={`pointer-events-none absolute z-[60] rounded-sm border border-white/10 bg-black/90 px-2 py-1 text-[11px] leading-tight whitespace-nowrap text-white shadow-lg opacity-0 group-hover/tip:opacity-100 group-hover/tip:animate-tooltip group-hover/tip:[animation-delay:200ms] group-focus-within/tip:opacity-100 motion-reduce:group-hover/tip:animate-none ${POSITIONS[side]} ${
                    side === "bottom"
